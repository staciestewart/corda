...
The responder flow, which will automically run on the other participants' nodes, just has to call ``ReceiveFinalityFlow``
...
For flows which are already initiating counterpart flows then it's a simple matter of using the exising flow session.
